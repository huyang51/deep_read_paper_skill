...
**关联方式**：
1. 在当前 paper 的 YAML frontmatter `related_papers` 字段中添加关联论文的 ID
2. 在当前 paper body 中，引用旧论文时使用**加粗文本**（如 `**PreFLMR**`），不使用 wikilink——系统会自动在旧论文中创建反向 wikilink 来生成图谱箭头
3. 如有有价值的跨论文创新见解，在 vault 的 `insights/` 下创建 insight 文件
4. **箭头方向**：系统自动在旧论文 body 中创建 `## 后续引用` 小节，包含指向新论文的 `[[wikilink]]`，使图谱箭头从旧论文指向新论文（学术影响流向：早期工作 → 后续工作）
...
- **必须**在新论文的 `related_papers` frontmatter 中添加关联旧论文的 ID
- 确认系统已在旧论文 body 中自动创建了 `## 后续引用` 小节（包含指向新论文的 `[[wikilink]]`）
