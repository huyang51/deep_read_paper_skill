...
- **重点精读**：摘要、引言末尾（贡献列表）、**相关工作**（理解方法定位和对比基准，是方法溯源分析的基础）、方法章节、实验章节开头与结果表、结论、**附录中的消融实验与训练细节**
- **快速浏览**：补充材料、参考文献
...
对于论文提出的方法，用通俗易懂的语言进行解读：

- **方法全流程概览**：按执行顺序完整叙述方法的全部流程（输入 → 各阶段/组件 → 输出），用连贯的叙述性语言让读者获得方法的"森林视图"。覆盖所有主要组件，标注图表/算法编号以追溯原文，提及但不展开公式细节（留待"关键公式解读"）
- **核心直觉**：先用一句话/一个类比解释方法的核心思想（就像向非专业人士解释）
...
- **关键公式解读**：对每个关键公式，分别给出：
  ...
  - 为什么这样设计（而不用其他形式）
- **训练/推理流程**：简要描述方法在实际使用时的流程，包括训练时的输入-前向-损失-反向和推理时的输入-前向-输出
